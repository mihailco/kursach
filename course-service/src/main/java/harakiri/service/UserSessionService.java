...
public class UserSessionService {
    ...
    public BeginTestResponse beginTest(String testId, Long userId) throws AccessDeniedException {
        ...
        testCollection.getQuestionList().forEach(this::shufflequestionOptions);
    ...
    private void shufflequestionOptions(Question question) {
        if (question.getJuxtapositionOption() != null && question.getJuxtapositionOption().getRightOption() != null)
            Collections.shuffle(question.getJuxtapositionOption().getRightOption());

        if (question.getChooseOption() != null) {
            Collections.shuffle(question.getChooseOption());
        }

        if (question.getSequenceOptions() != null) {
            Collections.shuffle(question.getSequenceOptions());
        }
    }
}